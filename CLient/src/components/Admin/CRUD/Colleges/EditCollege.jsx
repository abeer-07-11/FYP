...
export default function EditColleges() {
  ...
  const { name, degree, fee, rank, website } = college;
  ...
  const onSubmit = async e => {
    e.preventDefault();
    await axios.put(`http://localhost:5000/colleges/${id}`, college);
    history.push('/college');
  };
  const loadCollege = async () => {
    const result = await axios.get(`http://localhost:5000/colleges/${id}`);
          ...
          <Grid container spacing={3}>
            <Grid item xs={12} md={10} lg={8}>
                <form >
                    ...
                    <TextField
                    id="outlined-basic"
                    label="Enter the Rank of the College"
                    name="rank"
                    variant="outlined"
                    value={rank}
                    onChange={e => onInputChange(e)}
