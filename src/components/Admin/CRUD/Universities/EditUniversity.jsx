...
export default function EditUniversity() {
  ...
  const { name, degree, fee, rank, website } = university;
  ...
  const onSubmit = async e => {
    e.preventDefault();
    await axios.put(`http://localhost:5000/universities/${id}`, university);
    history.push('/university');
  };
  const loadUniversity = async () => {
    const result = await axios.get(`http://localhost:5000/universities/${id}`);
