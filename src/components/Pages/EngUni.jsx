...
import Eng from '../../Images/engineering.jpg';
...
const CsUni = () => {
    ...
    useEffect(() => {
      ...
      const loadeu = async () => {
        const result = await axios.get("http://localhost:5000/universities?department=engineering");
        seteu(result.data);
      };

    return ( 
        <div>
            <Title title="Universities & Degrees" sub="Related to Engineering" bg={Eng} />
            <table class="table-auto ml-12">
                ...
                <tbody>
                    {eu.map((eu) => (
                    <tr>
                        <td class="border px-4 py-2">{eu.name}</td>
                        <td class="border px-4 py-2">{eu.degree}</td>
                        <td class="border px-4 py-2">{eu.fee}</td>
                        <td class="border px-4 py-2">{eu.rank}</td>
                        <td class="border px-4 py-2">{eu.website}</td>
                    </tr>
                    ))}
                    {art.map((Art) => (
                    <tr>
